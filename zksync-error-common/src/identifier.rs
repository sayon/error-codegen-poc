...
impl Identifier {
    ...
    pub fn decode(raw_code: i32) -> Option<Self> {
        ...
        let kind: Kind = match domain {
            DomainCode::Compiler => Kind::Compiler(CompilerComponentCode::from_repr(component_code)?),
            DomainCode::Tooling => Kind::Tooling(ToolingComponentCode::from_repr(component_code)?),
        };
